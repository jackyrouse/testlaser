...
-------------------------------------------------------------*/
bool
CYdLidar::doProcessSimple(LaserScan &outscan, bool &hardwareError)
{
    hardwareError = false;
        ...
        if (op_result == RESULT_OK)
        {
            if (compensate)
            {
                ...
                for (; i < count; i++)
                {
                    ...
                    {
                        float angle =
                            (float) ((nodes[i].angle_q6_checkbit >> LIDAR_RESP_MEASUREMENT_ANGLE_SHIFT) / 64.0f);
                ...
                for (size_t i = 0; i < NODE_COUNTS; i++)
                {
                    ...
                    intensity =
                        (float) (angle_compensate_nodes[i].sync_quality >> LIDAR_RESP_MEASUREMENT_QUALITY_SHIFT);
                ...
                scan_msg.config.ang_increment =
                    (scan_msg.config.max_angle - scan_msg.config.min_angle) / (double) NODE_COUNTS;
...
bool
CYdLidar::getDeviceHealth() const
{
    if (!YDlidarDriver::singleton())
        return false;
...
bool
CYdLidar::getDeviceDeviceInfo() const
{
    if (!YDlidarDriver::singleton())
        return false;
    ...
    switch (devinfo.model)
    {
        ...
        case 5:
        {
            ...
            switch (_rate.rate)
            {
                case 0:fprintf(stdout, "Current Sampling Rate : 4K\n");
                    break;
                case 1:NODE_COUNTS = 1440;
                    EACH_ANGLE = 0.25;
                    fprintf(stdout, "Current Sampling Rate : 8K\n");
                    break;
                case 2:NODE_COUNTS = 1440;
                    EACH_ANGLE = 0.25;
...
void
CYdLidar::setSerialPort(const std::string &port_name)
{
    if (YDlidarDriver::singleton())
    {
...
void
CYdLidar::setSerialBaud(const uint32_t baudrate)
{
    if (YDlidarDriver::singleton())
    {
...
void
CYdLidar::setIntensities(bool intensity)
{
    if (YDlidarDriver::singleton())
    {
...
void
CYdLidar::setMaxRange(const float range)
{
    max_range = range;
}

void
CYdLidar::setMinRange(const float range)
{
    min_range = range;
}
